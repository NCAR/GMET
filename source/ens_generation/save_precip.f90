...
!a          if (file_times(1) == times(1)) then
!a            trec = i
!a          end if
!a        end do
!a        if (trec == 0) then
...
!a  end if
  ! --- end IF condition for whether file exists or not ---
...
!
 ...
 & error)
  if (error /= 0) return
!
  call check (nf90_put_var(ncid, pop_varid, pop, start=start3, count=count3), "put pop error", &
 & error)
  if (error /= 0) return
!
  call check (nf90_put_var(ncid, pcp_error_varid, pcperror, start=start3, count=count3), "put pcp_e&
 &rror error", error)
  if (error /= 0) return
!
