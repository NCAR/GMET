...
IF (ALLOCATED(GMSK)) THEN
  ...
  IF (IERR.NE.0) &
    CALL EXIT_SCRF(1,' problem deallocating space for mask ')
END IF
ALLOCATE(GMSK(NSPL1,NSPL2), STAT=IERR)
IF (IERR.NE.0) &
  CALL EXIT_SCRF(1,' problem allocating space for mask ')
! nullify pointers
...
IF (ASSOCIATED(SPCORR)) THEN
  ...
  IF (IERR.NE.0) &
    CALL EXIT_SCRF(1,' problem deallocating space for the spatial correlation structure ')
END IF
ALLOCATE(SPCORR(NSPL1,NSPL2), STAT=IERR)
IF (IERR.NE.0) &
  CALL EXIT_SCRF(1,' problem allocating spatial corr struct ')
IF (INIT) THEN
 DO ISP1=1,NSPL1
  DO ISP2=1,NSPL2
...
IF (ASSOCIATED(IORDER)) THEN
  ...
  IF (IERR.NE.0) &
    CALL EXIT_SCRF(1,' problem deallocating space for the processing order ')
END IF
...
IF (IERR.NE.0) &
  CALL EXIT_SCRF(1,' problem allocating space for the processing order ')
! ----------------------------------------------------------------------------------------
  ...
  DO ISP2=1,NSPL2,INCR
   ...
   IF (.NOT.GMSK(ISP1,ISP2)) THEN
    ...
    IF (ALLOCATED(IPOS)) THEN
      ...
      IF (IERR.NE.0) &
        CALL EXIT_SCRF(1,' problem deallocating ipos ')
    END IF
    IF (ALLOCATED(JPOS)) THEN
      DEALLOCATE(JPOS, STAT=IERR)
      IF (IERR.NE.0) &
        CALL EXIT_SCRF(1,' problem deallocating jpos ')
    END IF
    IF (ALLOCATED(WGHT)) THEN
      DEALLOCATE(WGHT, STAT=IERR)
      IF (IERR.NE.0) &
        CALL EXIT_SCRF(1,' probelm deallocating wght ')
    END IF
    ALLOCATE(IPOS(MAXP),JPOS(MAXP), STAT=IERR)
    IF (IERR.NE.0) &
      CALL EXIT_SCRF(1,' problem allocating (i,j) position ')
    ...
    DO JSP1=MAX(1,ISP1-(INCR*NLOC)),MIN(ISP1+(INCR*NLOC),NSPL1)
     DO JSP2=MAX(1,ISP2-(INCR*NLOC)),MIN(ISP2+(INCR*NLOC),NSPL2)
      ...
      IF (GMSK(JSP1,JSP2)) THEN  ! local point has been previously generated
       ...
       IPOS(K) = JSP1
       JPOS(K) = JSP2
      ENDIF   ! if the point has been previously generated
     END DO  ! jsp1
    END DO  ! jsp2
    ...
    IF (K.GE.2) THEN
     ...
     IF (ALLOCATED(CORR)) THEN
       ...
       IF (IERR.NE.0) &
         CALL EXIT_SCRF(1,' probelm deallocating corr ')
     END IF
     IF (ALLOCATED(GVEC)) THEN
       DEALLOCATE(GVEC, STAT=IERR)
       IF (IERR.NE.0) &
         CALL EXIT_SCRF(1,' probelm deallocating gvec ')
     END IF
     IF (ALLOCATED(TWGT)) THEN
       DEALLOCATE(TWGT, STAT=IERR)
       IF (IERR.NE.0) &
         CALL EXIT_SCRF(1,' probelm deallocating twgt ')
     END IF
     IF (ALLOCATED(INDX)) THEN
       DEALLOCATE(INDX, STAT=IERR)
       IF (IERR.NE.0) &
         CALL EXIT_SCRF(1,' probelm deallocating indx ')
     END IF
     ALLOCATE(CORR(K-1,K-1),GVEC(K-1),TWGT(K-1),INDX(K-1), STAT=IERR)
     IF (IERR.NE.0) &
       CALL EXIT_SCRF(1,' problem allocating space for corr, gvec, twgt, or indx ')
     ...
     DO IPREV=1,K
      DO JPREV=1,IPREV
       ...
       ELSE
        ...
        ENDIF    ! switch between corr and gvec
       ENDIF    ! if the points are the same 
      END DO   ! jprev 
     END DO   ! iprev
     ! ------------------------------------------------------------------------------------
    ...
    IF (ASSOCIATED(SPCORR(ISP1,ISP2)%IPOS)) THEN
      ...
      IF (IERR.NE.0) &
        CALL EXIT_SCRF(1,' problem deallocating space for ipos, jpos ')
    END IF
    ...
    IF (IERR.NE.0) &
      CALL EXIT_SCRF(1,' problem allocating space for the (i,j) position of previously generated points ')
    ...
    IF (ASSOCIATED(SPCORR(ISP1,ISP2)%WGHT)) THEN
      ...
      IF (IERR.NE.0) &
        CALL EXIT_SCRF(1,' problem deallocating space for weights assigned to previously generated points ')
    END IF
    ALLOCATE(SPCORR(ISP1,ISP2)%WGHT(NPTS-1), STAT=IERR)
    IF (IERR.NE.0) &
      CALL EXIT_SCRF(1,' problem allocating space for weights assigned to previously generated points ')
    ...
    IF (ALLOCATED(WGHT)) THEN
      DEALLOCATE(WGHT, STAT=IERR)
      IF (IERR.NE.0) &
        CALL EXIT_SCRF(1,' problem deallocating wght` ')
    END IF
   ENDIF   ! if the point has not been generated yet
  END DO  ! ilat 
 END DO  ! ilon
END DO  ! ires
! deallocate mask
