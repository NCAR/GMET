...
!BL
	...
	if (lenran > 0) then
        ...
        ! AWW: rewrote this part to call a new function that provides a new seed 
        !      to kick the whole process off
        !	ranseeds(new:,1)=seq       # REPLACED WITH FOLLOWING
        if(seq == 0) then
        	call init_rand_seed_I4B(seq)   ! supplies 'seq', a seed
                print*, '--- initialized with new seed ---', seq
        end if
        ! end addition 

       	ranseeds(new:,1)=seq

        ! orig -- propagate initial seed and change negs to pos values
