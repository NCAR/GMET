...
! input
  ...
  IF (IERR.NE.0) &
    CALL EXIT_SCRF(1,'CRAN: PROBLEM ALLOCATING SPACE')
  ! ----------------------------------------------------------------------------------------
  ...
  ELSE
    ...
    IF (IERR.NE.0) &
      CALL EXIT_SCRF(1,'problem allocating array [field_rand.f90]')
    ...
    IF (IERR.NE.0) &
      CALL EXIT_SCRF(1,'problem deallocating array [field_rand.f90]')
  ENDIF   ! (if not the first point)
  END DO  ! igrd
