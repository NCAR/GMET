SUBROUTINE save_vars (pcp, tmean, trange, nx, ny, grdlat, grdlon, grdalt, Times, file, error)
  ...
  IF (error /= nf90_noerr) THEN
   ...
   CALL check (nf90_def_var(ncid, PCP_ERROR_NAME, NF90_FLOAT, dimids3, pcp_error_varid), "pcp_error var def error", &
  & error)
   IF (error /= 0) RETURN
  ...
  CALL check (nf90_put_var(ncid, pop_varid, tmean, start=start3, count=count3), "put tmean error", error)
  IF (error /= 0) RETURN
  CALL check (nf90_put_var(ncid, pcp_error_varid, trange, start=start3, count=count3), "put trange error", error)
  IF (error /= 0) RETURN
